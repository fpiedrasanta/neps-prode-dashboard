...
const ImagesPage = () => {
  ...
  // Refs para valores estables sin causar re-renders ni cambios de referencia
  ...
  // IntersectionObserver estable (solo depende de refs, no de estados)
  useEffect(() => {
  ...
  // Si hay más datos pero el contenido no desborda, cargar automáticamente
  useEffect(() => {
  ...
  }, [images, loading, hasMore, loadImages]);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const newFiles = files.map(file => ({ file, name: file.name.replace(/\.[^/.]+$/, ""), preview: URL.createObjectURL(file) }));
      setUploadFiles(prev => [...prev, ...newFiles]);
    }
  };

  const removeFile = (index: number) => {
    setUploadFiles(prev => { URL.revokeObjectURL(prev[index].preview); return prev.filter((_, i) => i !== index); });
  };

  const updateFileName = (index: number, name: string) => {
    setUploadFiles(prev => { const updated = [...prev]; updated[index].name = name; return updated; });
  };

  const openEditModal = (image: Image) => { setEditingImage(image); setShowModal(true); };
  const openDeleteModal = (image: Image) => { setDeletingImage(image); setShowDeleteConfirm(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      pageRef.current = 1; setImages([]); setPage(1); setHasMore(true); hasMoreRef.current = true; loadImages(true);
    } catch (err) { setError('No se pudieron subir las imágenes. Intentá nuevamente.'); console.error(err); }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccess('Imagen eliminada correctamente'); setShowDeleteConfirm(false); setDeletingImage(null);
      setTimeout(() => setSuccess(null), 3000);
      pageRef.current = 1; setImages([]); setPage(1); setHasMore(true); hasMoreRef.current = true; loadImages(true);
    } catch (err) { setError('No se pudo eliminar la imagen. Intentá nuevamente.'); console.error(err); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="special-posts-page">
      <div className="page-header">
        ...
        <button className="btn btn-primary create-btn" onClick={openUploadModal}><Plus size={18} /> Subir Imágenes</button>
      </div>
      ...
      <div className="search-bar">
        <div className="search-input">
          <Search size={18} />
          <input type="text" placeholder="Buscar imagen por nombre..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>

      <div className="posts-container">
        {images.length === 0 && !loading ? (
          <div className="empty-state"><ImageIcon size={48} /><p>No se encontraron imágenes</p></div>
        ) : (
          <>
              ...
              {images.map(image => (
                <div key={image.id} className="image-card">
                  ...
                  <div className="image-actions">
                    <button className="icon-btn edit" onClick={() => openEditModal(image)}><Edit2 size={14} /></button>
                    <button className="icon-btn delete" onClick={() => openDeleteModal(image)}><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal post-modal">
                    ...
                    <div className="files-list">
                      {uploadFiles.map((item, index) => (
                        <div key={index} className="file-item">
                          ...
                          <div className="file-info"><input type="text" value={item.name} onChange={(e) => updateFileName(index, e.target.value)} placeholder="Nombre para la imagen" className="file-name-input" /><div className="file-original-name">{item.file.name}</div></div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Subiendo...' : 'Subir Imágenes'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-body confirm-body">
              ...
              <h4>¿Estás seguro?</h4>
              <p>Vas a eliminar la imagen <strong>{deletingImage?.name || deletingImage?.fileName}</strong>. Esta acción no se puede deshacer.</p>
            </div>
            <div className="modal-footer">
